...
export async function listSessions({ page, pageSize, q, archived }) {
    ...
    const { count } = await db.get(
        `SELECT COUNT(1) as count FROM sessions WHERE ${where} AND COALESCE(created_by, '') LIKE ?`,
        filter,
    );
...
export async function getSession(sessionId) {
    ...
    const sessionRow = await db.get(
        `SELECT id, is_active as isActive, created_at as createdAt FROM sessions WHERE id = ?`,
        sessionId,
    );
...
export async function endSession(sessionId) {
    ...
    const { cnt } = await db.get(
        `SELECT COUNT(1) as cnt FROM players WHERE session_id = ? AND is_active = 1`,
        sessionId,
    );
    if (Number(cnt) > 0) {
        const err = new Error(
            'Cannot end session while there are active players. Cash out all players first.',
        );
        err.status = 400;
        throw err;
    }
...
export async function cashOut({ sessionId, playerId, finalChipCount }) {
    ...
    await assertSessionActive(db, sessionId);
    const player = await db.get(
        `SELECT net_balance as netBalance FROM players WHERE id = ? AND session_id = ?`,
        playerId,
        sessionId,
    );
    if (!player) {
