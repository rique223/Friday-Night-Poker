...
import { Language, Currency, TranslationKeys, TranslationKey } from '../types';
...
const dictionaries: Dictionaries = {
	...
	es: {
		...
		back: 'Volver',
		orderedNewest: 'Las sesiones están ordenadas de la más reciente a la más antigua dentro de cada grupo.',
		of: 'de',
		groupBy: 'Agrupar por',
		currencyBRL: 'Real',
		currencyUSD: 'Dólar',
...
export function PreferencesProvider({ children }: { children: React.ReactNode }) {
	...
	const [currency, setCurrency] = useState<Currency>(() => (localStorage.getItem('currency') as Currency) || 'BRL');
	...
	const formatCurrency = useMemo(() => {
		return (value: number) =>
			new Intl.NumberFormat(lang === 'pt' ? 'pt-BR' : lang === 'es' ? 'es-ES' : 'en-US', { style: 'currency', currency }).format(
				value,
			);
	}, [lang, currency]);

	const value = useMemo(() => ({ lang, setLang, currency, setCurrency, t, formatCurrency }), [lang, currency, t, formatCurrency]);
